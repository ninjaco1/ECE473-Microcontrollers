...
//  Author: Anthony Nguyen, Don Nguyen, Triet Nguyen
...
int main(void)
{
  ...
  while (1)
  {
    ...
    case SR_RANDOM_DELAY:
    {
      ...
      // Set up TC0
      TCCR0 |= (0 << WGM01) | (0 << WGM00) | (0 << CS02) | (0 << CS01) | (0 << CS00); // set TC0 timer into normal mode and disable clock
      TCNT0 = 0x00;
      TIMSK &= (0 << TOIE0); // disable TC0 interrupts
    ...
    case SR_TIMING_USER:
    {
      ...
      TIMSK &= (0 << TOIE1);                                                                          // disable TC1 interrupts in TIMSK
      ...
      while (((TIFR & (1 << TOV1)) == 0) && (PIND == 0xFF))
      {
      }; // wait until button pressed or TC1 OVF set
      numticks = TCNT1;
